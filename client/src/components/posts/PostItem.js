  ...
  return (
    <div className='post bg-gradient-to-r from-orange-700/5 to-slate-950/95 hover:shadow-lg hover:shadow-black rounded-xl p-1 my-1 text-light'>
      ...
      <div>
        ...
        <button onClick={(e) => dispatch(addLike(_id))} type='button' className='btn btn-sm bg-light hover:bg-gray-300'>
          <i className='fas fa-thumbs-up'></i>
          <span>{likes.length > 0 && <span>{likes.length}</span>}</span>
        </button>
        <button onClick={(e) => dispatch(removeLike(_id))} type='button' className='btn btn-sm bg-light hover:bg-gray-300'>
